...
void main() {
  ...
  group('ApiClient', () {
    test(
      'coalesces concurrent identical GET requests into one network call',
      () async {
    ...
    test(
      'uses background decode and parses mixed-case offer payloads',
      () async {
        final storage = await _TestStorageHarness.create();
        addTearDown(storage.dispose);
    ...
    test(
      'returns a fresh cached value without a second network call',
      () async {
        final storage = await _TestStorageHarness.create();
        addTearDown(storage.dispose);
    ...
    test(
      'watchParsed emits stale cached data first and then fresh data',
      () async {
        final storage = await _TestStorageHarness.create();
        addTearDown(storage.dispose);
    ...
    test(
      'reuses the same nearby-offers cache key for small location drift',
      () async {
        final storage = await _TestStorageHarness.create();
        addTearDown(storage.dispose);
    ...
    test(
      'watchParsed suppresses the second emission when payload is unchanged',
      () async {
        final storage = await _TestStorageHarness.create();
        addTearDown(storage.dispose);
    ...
    test(
      'returns cached data when revalidation fails and cache exists',
      () async {
        final storage = await _TestStorageHarness.create();
        addTearDown(storage.dispose);
        ...
        final client = _CountingHttpClient((request) async {
          return _jsonResponse({'message': 'Server blew up'}, statusCode: 500);
        });
    ...
    test(
      'throws when the network fails and there is no cache to fall back to',
      () async {
        final storage = await _TestStorageHarness.create();
        addTearDown(storage.dispose);
...
class _TestStorageHarness {
  ...
  static Future<_TestStorageHarness> create() async {
    ...
    return _TestStorageHarness._(
      StorageService(prefs: prefs, responseCache: responses, routes: routes),
...
http.StreamedResponse _jsonResponseString(String body, {int statusCode = 200}) {
