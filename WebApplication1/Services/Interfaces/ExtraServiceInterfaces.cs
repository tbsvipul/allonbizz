...
{
    Task<PagedResponse<ReviewDto>> GetReviewsAsync(Guid? shopId, Guid? offerId, PaginationParams paging, bool publishedOnly = false);
    Task ReplyToReviewAsync(Guid reviewId, ReviewReplyDto dto);
    Task SubmitReviewAsync(Guid userId, SubmitReviewDto dto);
}

public interface ILoyaltyService
{
    Task<LoyaltyProgramDto> GetLoyaltyProgramAsync(Guid shopId);
    Task<LoyaltyProgramDto> ManageLoyaltyProgramAsync(UpdateLoyaltyProgramDto dto);
}
