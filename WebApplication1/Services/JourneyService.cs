...
public class JourneyService : IJourneyService
{
    ...
    public async Task<List<JourneyRecommendationResponse>> GetNearByShopsAsync(Guid journeyId, double lat, double lng, double radiusKm = 5)
    {
        ...
        var allShops = await _db.Shops.Include(s => s.Category).Where(s => s.IsActive).ToListAsync();

        var recommendations = allShops
            .Select(s => new {
                Shop = s,
                Distance = CalculateDistance(lat, lng, s.Latitude ?? 0, s.Longitude ?? 0),
            ...
            .Select(x => new JourneyRecommendationResponse
            {
                ...
                Latitude = x.Shop.Latitude ?? 0,
                Longitude = x.Shop.Longitude ?? 0,
