...
public class AnalyticsService : IAnalyticsService
{
    ...
    {
        var users = await _userRepo.Query().AsNoTracking().ToListAsync(ct);
        users = users
            .Where(user => !dto.DateFrom.HasValue || user.CreatedAt >= dto.DateFrom.Value)
            .Where(user => !dto.DateTo.HasValue || user.CreatedAt <= dto.DateTo.Value)
            .Where(user => !TryGetFilter(dto.Filters, "role", out var role) || user.Role.Equals(role, StringComparison.OrdinalIgnoreCase))
            .Where(user => !TryGetFilter(dto.Filters, "status", out var status) || user.Status.ToString().Equals(status, StringComparison.OrdinalIgnoreCase))
            .ToList();
    ...
    private async Task<CustomAnalyticsReportDto> BuildShopReportAsync(GenerateCustomReportDto dto, List<string> metrics, string? groupBy, CancellationToken ct)
    {
        ...
        shops = shops
            .Where(shop => !dto.DateFrom.HasValue || shop.CreatedAt >= dto.DateFrom.Value)
            .Where(shop => !dto.DateTo.HasValue || shop.CreatedAt <= dto.DateTo.Value)
            .Where(shop => !TryGetFilter(dto.Filters, "status", out var status) || ResolveShopStatus(shop).Equals(status, StringComparison.OrdinalIgnoreCase))
            .Where(shop => !TryGetFilter(dto.Filters, "category", out var category) || (shop.Category?.Name ?? "uncategorized").Equals(category, StringComparison.OrdinalIgnoreCase))
            .ToList();
