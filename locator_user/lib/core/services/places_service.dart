...
class PlacesService {
  static const String _legacyLocatorApiBaseUrl =
  ...
  Future<List<PlaceSuggestion>> _fetchSuggestionsFromApis(String query) async {
    ...
    final candidateBaseUrls = <String>[
      _normalizeBaseUrl(_apiClient.baseUrl),
      _normalizeBaseUrl(
        dotenv.env['PLACES_API_BASE_URL'] ?? _legacyLocatorApiBaseUrl,
      ),
    ];
  ...
  Future<List<PlaceSuggestion>> _fetchPhotonSuggestions(String query) async {
    final url = Uri.parse(_photonSearchUrl).replace(
  ...
  Future<PlaceSuggestion?> reverseGeocode(double lat, double lon) async {
    final url = Uri.parse('https://photon.komoot.io/reverse').replace(
      queryParameters: {
